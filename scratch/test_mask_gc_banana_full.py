...
def test_mask_grabcut_banana_full():
    ...
    if os.path.exists(banana_frame):
        ...
        # Bounding box wide enough to contain full banana but not the entire water bottle
        x1, y1 = int(sw * 0.12), int(sh * 0.28)
        x2, y2 = int(sw * 0.78), int(sh * 0.65) # Extended x2 slightly
        ...
        # Mark blue labels (water bottle label) as definitely background
        ...
        cv2.imwrite("scratch/test_mask_grabcut_banana_full.png", segmented)
        print("Saved full banana GrabCut to scratch/test_mask_grabcut_banana_full.png")

if __name__ == "__main__":
    test_mask_grabcut_banana_full()
