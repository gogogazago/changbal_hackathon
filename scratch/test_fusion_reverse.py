...
def test_reverse_fusion(object_name, sweep_angle_deg=100):
    frames_dir = f"pixel_frames/{object_name}/frames"
    frame_files = sorted([f for f in os.listdir(frames_dir) if f.startswith("frame_")])
    
    # We will sample 6 key frames across the sequence
    ...
    for i, idx in enumerate(sample_indices):
        ...
        # Test REVERSED sweep direction (negative sign)
    ...
    cv2.imwrite(f"scratch/test_fusion_reversed_{object_name}.png", canvas)
    print(f"Saved reversed fused render to scratch/test_fusion_reversed_{object_name}.png")

if __name__ == "__main__":
    test_reverse_fusion("headphone", 90)
    test_reverse_fusion("banana", 90)
