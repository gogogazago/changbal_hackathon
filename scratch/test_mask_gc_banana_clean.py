...
def test_mask_grabcut_banana_clean():
    ...
    if os.path.exists(banana_frame):
        ...
        # Bounding box cropped on the right to exclude the water bottle
        # Banana center is on the left-middle. Bottle is on the right.
        x1, y1 = int(sw * 0.15), int(sh * 0.28)
        x2, y2 = int(sw * 0.70), int(sh * 0.65) # Narrower box to exclude bottle
        ...
        # Inside bounding box, use yellow HSV color
        ...
        # Also mark blue pixels (water bottle label) as definitely background
        ...
        cv2.imwrite("scratch/test_mask_grabcut_banana_clean.png", segmented)
        print("Saved cleaned GrabCut banana test to scratch/test_mask_grabcut_banana_clean.png")

if __name__ == "__main__":
    test_mask_grabcut_banana_clean()
